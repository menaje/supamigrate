...
export async function getTablesForDataMigration(): Promise<TableInfo[]> {
  ...
  for (const row of result.rows) {
    ...
    for (const r of columnsResult.rows) {
      columnTypes.set(r.column_name, r.udt_name);
    }
    ...
    while (offset < table.rowCount) {
      ...
      for (let rowIdx = 0; rowIdx < sourceData.rows.length; rowIdx++) {
        ...
        for (let colIdx = 0; colIdx < table.columns.length; colIdx++) {
          ...
          const colType = table.columnTypes.get(colName) || '';
          let val = row[colName];

          // JSONB/JSON columns: stringify objects to prevent pg driver
          // from converting JS arrays to PostgreSQL array literals
          if ((colType === 'jsonb' || colType === 'json') && val !== null && typeof val === 'object') {
